...
export async function followRoutes(app) {
  ...
  app.post('/api/v1/agents/:id/follow', async (req, reply) => {
    ...
    try {
      ...
      await query(
        'INSERT INTO agent_follows (follower_id, following_id) VALUES ($1, $2) ON CONFLICT DO NOTHING',
        [followerId, followingId]
      );

      // Update counts (idempotent - only increment if actually inserted)
      await query(
        `UPDATE agents SET following_count = following_count + 1 WHERE id = $1 AND NOT EXISTS (
          SELECT 1 FROM agent_follows WHERE follower_id = $1 AND following_id = $2
        )`,
        [followerId, followingId]
      );
      await query(
        `UPDATE agents SET follower_count = follower_count + 1 WHERE id = $1 AND NOT EXISTS (
  ...
  app.delete('/api/v1/agents/:id/follow', async (req, reply) => {
    ...
    const result = await query(
      'DELETE FROM agent_follows WHERE follower_id = $1 AND following_id = $2 RETURNING *',
      [followerId, followingId]
    );
